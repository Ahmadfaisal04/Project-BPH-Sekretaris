...
const menuItems = [
  ...
  { text: "Surat Masuk", icon: <MoveToInboxIcon />, path: "/suratmasuk" },
  { text: "Surat Keluar", icon: <OutboxIcon />, path: "/suratkeluar" },
...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      console.log("Dashboard layout - Token:", token ? "ada" : "tidak ada");
      console.log(
        "Dashboard layout - User data:",
        userData ? "ada" : "tidak ada"
      );

      if (!token || !userData) {
        console.log(
          "Dashboard layout - Mengarahkan ke login karena tidak ada token/user"
        );
        router.push("/login");
        return;
      }
      ...
      try {
        ...
        setUser(parsedUser);
        console.log("Dashboard layout - User berhasil di-set:", parsedUser);
      } catch (error) {
        console.error("Dashboard layout - Error parsing user:", error);
        router.push("/login");
      }
    }
  }, [router]);
  ...
  const handleLogout = () => {
    ...
    console.log("Dashboard layout - Logout berhasil, mengarahkan ke login");
    router.push("/login");
  };
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          <Box>
            ...
            <Typography
              variant="caption"
              sx={{ color: darkMode ? "#fff" : colors.text.secondary }}
            >
        ...
        {menuItems.map((item) => (
          <ListItem
            ...
            sx={{
              ...
              // '&.Mui-selected': {
              //   // bgcolor: darkMode ? 'rgba(255, 255, 255, 0.1)' : 'linear-gradient(135deg, #2C7DA0 0%, #014F86 100%)',
              //   color: '#fff',
              //   '& .MuiListItemIcon-root': { color: '#fff' },
              // },
              // '&:hover': {
              //   // bgcolor: darkMode ? 'rgba(255, 255, 255, 0.05)' : 'linear-gradient(135deg, #2C7DA0 0%, #014F86 100%)',
              //   color: '#fff',
              //   '& .MuiListItemIcon-root': { color: '#fff' },
              // },
            }}
  ...
  return (
    <Box
      sx={{
        display: "flex",
        bgcolor: darkMode ? "#2f2f2f" : "#F8F9FA",
        minHeight: "100vh",
          ...
          <Container maxWidth="lg">
            <Grid container justifyContent="space-between">
              <Grid item xs={12} sm={6}>
                ...
                >
                  © {new Date().getFullYear()} Persuratan Sekretaris COCONUT
                </Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Box
